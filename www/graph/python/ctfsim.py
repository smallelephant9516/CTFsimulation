...
app.config.update({
    ...
    'requests_pathname_prefix': '/graph/'
})
server = app.server
...
app.layout = html.Div([
    ...
    html.Div([
        ...
        dcc.Input(id='defocus_input', value=0.5, type='number',debounce=True,style={'float': 'left', 'width': '10%','margin-top':20,'margin-right':10}),
        html.Div([
            dcc.Slider(
                id='defocus_slide',
    ...
    html.Div([
            ...
            html.Div([
                ...
                dcc.Input(id='q', value=0.05, type='number',debounce=True,style={'margin-right':50,'float':'left','width': '120%'}),
            ], className="two columns"),
            html.Div([
                html.Label('Sample step', style={'margin-right':20,'float':'left','width': '150%'}), #chromatic aberration coefficient(mm)
                dcc.Input(id='step', value=0.005, type='number',debounce=True,style={'margin-right':50,'float':'left','width': '120%'}), #chromatic aberration coefficient(mm)
            ], className="two columns"),
            html.Div([
                html.Label('Energy (keV)',style={'margin-right':20,'float':'left','width': '150%'}),
                dcc.Input(id='E', value=300, type='number',debounce=True,style={'margin-right':50,'float':'left','width': '120%'}), # Energy(keV)
            ], className="two columns"),
              ],className="six columns"),
    html.Div([
            html.Div([
                html.Label('Spherical aberration (mm)',style={'margin-right':0,'margin-left':0,'font-size':10}),
                dcc.Input(id='c', value=2.7, type='number',debounce=True,style={'margin-right':0,'margin-left':0,'width': '120%'}), #Spherical aberration(mm)
            ], className="two columns"),
            html.Div([
                html.Label('B factor',style={'margin-right':0}),
                dcc.Input(id='B', value=100, type='number',debounce=True,style={'margin-right':0,'width': '120%'}), #drift
            ], className="two columns"),
            html.Div([
                html.Label('A/pix',style={'margin-right':0}),
                dcc.Input(id='apix', value=1, type='number',debounce=True,style={'margin-right':0,'width': '120%'}), #apix
            ], className="two columns"),
              ],className="six columns"),
])
...
def data_update(d_ctf,n_clicks):
    global d
    print()
    if n_clicks == 0:
        d['0']=d_ctf
...
@app.callback(Output('change_select', 'children'),
              [Input('trace_num', 'value'),
               Input('frame', 'children'),
               Input('d_ctf', 'children'),])
def change_select(trace_num, frame, d_ctf):
    print(trace_num)
    a=trace_num
...
def display_figure(change_select):
    ...
    return {
            ...
            'layout': go.Layout(
            xaxis={
                'title': 'Spatial frequency 1/A', 'range':[0,0.5]
            },
